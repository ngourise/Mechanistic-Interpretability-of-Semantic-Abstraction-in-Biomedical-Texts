...
# ------------------ Colab Setup ------------------
!pip install torch transformers pandas matplotlib sacremoses
import time
...
# Upload CSV
print("Please upload your test.csv file (with columns: 'input_text', 'target_text')")
uploaded = files.upload()

#if not uploaded:
#    raise FileNotFoundError("No file uploaded.")
DATA_CSV = "test.csv"   #Use test.csv file whether it was just uploaded or it already exists in the colab runtime
...
MAX_PAIRS = None           # set to None for all
...
def block_parts(block: nn.Module, arch: str) -> Dict[str, Optional[nn.Module]]:
    ...
    if hasattr(block, "layer") and isinstance(block.layer, (list, tuple)):
        # T5 has layer[0] = Attention, layer[1] = FFN
        sa = getattr(block.layer[0], "SelfAttention", getattr(block.layer[0], "self_attn", block.layer[0]))
        out["attn"] = sa   # return the whole attention module
        ff = getattr(block.layer[-1], "DenseReluDense", block.layer[-1])
        out["mlp"] = ff
        return out
...
def num_heads(attn_module: nn.Module) -> Optional[int]:
    for attr in ["num_attention_heads", "n_head", "num_heads", "n_heads"]:
        if hasattr(attn_module, attr):
            return int(getattr(attn_module, attr))
    # gpt2 attn.c_attn has heads on config
    cfg = getattr(attn_module, "config", None)
    if cfg is not None and hasattr(cfg, "n_head"):
...
def collect_donor_activations(
    ...
    def make_attn_hook(li: int, attn_mod: nn.Module):
        ...
        def hook(_m, _in, out):
            y = out[0] if isinstance(out, (list, tuple)) else out   # [B,T,H]
            bank.put("attn", li, y)
...
def apply_patches_and_run(
    ...
    def make_attn_patch(li: int, attn_mod: nn.Module):
        ...
        def hook(_m, _in, out):
            ...
            return (y,) + tuple(out[1:]) if isinstance(out, (list, tuple)) else y
        return hook
...
def run_component_sweep(
    ...
    # impact score, per your spec
    if arch == "encoder":
        df["impact_score"] = df["to_target_cosine"]
    else:
        df["impact_score"] = -df["patched_ppl"]
    return df
    ...
    for j, mode in enumerate(modes):
        ...
        if not mlp.empty:
            ...
            if y_vals.size:
                ...
                pad = 0.1 * (ymax - ymin if ymax > ymin else max(abs(ymax), 1e-3))
                axes[1, j].set_ylim(ymin - pad, ymax + pad)
        else:
...
def run_full_experiment(
    ...
    for model_name in model_list:
        ...
        # Save CSVs
        csv_all = os.path.join(OUT_DIR, f"{sanitize(model_name)}__component_causal_results.csv")
        ...
        csv_top = os.path.join(OUT_DIR, f"{sanitize(model_name)}__topk.csv")
        ...
        try:
            plot_causal_heatmaps(by_model, model_name,
                                 save_path=os.path.join(OUT_DIR, f"{sanitize(model_name)}__heatmaps.png"))
            if by_model["delta_ppl"].notna().any():
                plot_delta_ppl(by_model, model_name,
                               save_path=os.path.join(OUT_DIR, f"{sanitize(model_name)}__delta_ppl.png"))
        except Exception as e:
